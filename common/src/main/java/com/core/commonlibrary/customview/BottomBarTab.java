...
public class BottomBarTab extends FrameLayout {
    ...
    public BottomBarTab(Context context, @DrawableRes int icon, @DrawableRes int icon_p, CharSequence title) {
        this(context, null, icon, icon_p, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int icon, int icon_p, CharSequence title) {
        this(context, attrs, 0, icon, icon_p, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int defStyleAttr, int icon, int icon_p, CharSequence title) {
        super(context, attrs, defStyleAttr);
        init(context, icon, icon_p, title);
    }

    private void init(Context context, int icon, int icon_p, CharSequence title) {
        this.micon = icon;
        this.micon_p = icon_p;
        mContext = context;
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);//66 74
//        params.topMargin=20;
        mIcon.setBackgroundResource(micon);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            mIcon.setBackgroundResource(micon_p);
            mTvTitle.setTextColor(ServiceFactory.getInstance().getCommonService().getColorPrimary());
        } else {
            mIcon.setBackgroundResource(micon);
